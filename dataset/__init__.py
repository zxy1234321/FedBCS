...
from dataset.utils.public_dataset import PublicDataset
...
def get_prive_dataset(args: Namespace) -> FederatedDataset:
    assert args.dataset in Priv_NAMES.keys(), f"Invalid dataset name '{args.dataset}'. Valid options are: {list(Priv_NAMES.keys())}"
    return Priv_NAMES[args.dataset](args)


def get_public_dataset(args: Namespace) -> PublicDataset:
    assert args.public_dataset in Pub_NAMES.keys(), f"Invalid dataset name '{args.public_dataset}'. Valid options are: {list(Pub_NAMES.keys())}"
